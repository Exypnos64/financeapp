...
export function validateTransaction(form: FormData): TransactionValidation {
    ...
    if (!userDate || typeof userDate !== "string") return { ok: false, failure: fail(422, {
...
export async function handleTransactionResponse(response: Response, form?: FormData): Promise<ActionFailure<TransactionFormFailure> | undefined> {
    if (response.ok) {
