<script lang="ts">
    ...
    // svelte-ignore state_referenced_locally
    let merchant = $state(Number(form?.values?.merchantId ?? transaction?.merchantId ?? merchants.find(m => m.name === "Unknown")?.id));
    // svelte-ignore state_referenced_locally
...
<form method="POST" action={action}>
    ...
    <pre id="originalStatement" style="display: inline;">{transaction?.originalStatement}</pre><br><br>

    <span>Original Date:</span>
    <pre id="originalDate" style="display: inline;">{transaction?.originalDate}</pre><br><br>

    <span>Last Modified Date (UTC):</span>
    <pre id="lastModifiedUtc" style="display: inline;">{transaction?.lastModifiedUtc}</pre><br><br>
    {/if}
